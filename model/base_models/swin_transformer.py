...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        x = x.view(B, H, W,
                   C)  ## 从1 3136 96 转为  1 56 56 96  注意这个时候就从输入的一维那种向量转为了特征图，也就是一维3136，到了一个二维特征图 56 56 ，对应到原始图片是224 224
        ...
        x_windows = x_windows.view(-1, self.window_size * self.window_size,
                                   C)  ## 64 49 96 ；64是bs乘以每个图片的窗口，49是一个窗口中的有多少个元素，对应到NLP中，就是有多少个单词，96是通道数，对应到NLP就是每个单词的维度  # nW*B, window_size*window_size, C

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows,
                                 mask=self.attn_mask)  ## attn_windows 64 49 96，和trm没区别哈，长度49不变，toen维度96没变；  # nW*B, window_size*window_size, C
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        img_size = to_2tuple(img_size)  # 返回一个元组
        # img_size = (360 // 2, 360 // 2)
        patch_size = to_2tuple(patch_size)  # 返回一个patch_size元组，即(4,4)
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]]  # [224//4,224//4]
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)  # B Ph*Pw C  1 3136 96  3136就是我们的56*56 压扁,特征图压扁，然后最后两个维度交换
...
if __name__ == '__main__':
    v = SwinTransformer(num_classes=19)
    ...
    print(preds.shape)
    # print(preds.reshape(1,-1,2,2).shape)
