...
def test(args, test_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(test_loader):
        # for i, (input, label, name) in enumerate(test_loader):
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="IDWRSeg_H", help="model name: [DSANet,SPFNet,SSFPN,DWRSeg,PPLiteSeg]")
    parser.add_argument('--dataset', default="camvid", help="dataset: cityscapes or camvid")
    parser.add_argument('--num_workers', type=int, default=1, help="the number of parallel threads")
    parser.add_argument('--batch_size', type=int, default=1,
                        help=" the batch_size is set to 1 when evaluating or testing")
    parser.add_argument('--checkpoint', type=str,
                        default="./checkpoint/camvid/IDWRSeg_Hbs4gpu1_trainval/model_221.pth",
