...
from utils.loss import CrossEntropyLoss2d, OhemCrossEntropy, OhemCELoss, FocalLoss2d
from utils.lr_scheduler import WarmupPolyLR
from utils.convert_state import convert_state_dict

GLOBAL_SEED = random.randint(1, 100)
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        print("[%d/%d]  time: %.2f" % (i + 1, total_batches, time_taken))  # 推理一张图片的时间
        output = output.cpu().data[0].numpy()
        # print('eval output.shape', output.shape)
        # output=output.data[0].numpy()
        gt = np.asarray(label[0].numpy(), dtype=np.uint8)  # [c,h,w]
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        # output1, sup1, sup2, sup3, sup4 = model(images)
        output = model(images)
        # print('output1.shape', output1.shape)
        # print('out2.shape', out2.shape)
        loss1 = criterion(output, labels)
        # loss2 = criterion(aux_1, labels)

        # loss3 = criterion(aux_2, labels)
        # loss4 = criterion(aux_3, labels)
        # loss5 = criterion(sup4, labels)
        loss = loss1
...
def train_model(args):
    ...
    if args.dataset == 'camvid':
        criteria = CrossEntropyLoss2d(weight=weight, ignore_label=11)
        # min_kept = int(args.batch_size // len(args.gpus) * h * w // 16)
        # criteria= OhemCELoss(weights=weight, ignore_label=ignore_label,
        # thresh=0.7, n_min=min_kept)
        # criteria = FocalLoss2d(weight=weight, ignore_index=ignore_label)
        # criteria = OhemCrossEntropy(weight=weight, ignore_label=ignore_label)
    elif args.dataset == 'cityscapes':
        min_kept = int(args.batch_size // len(args.gpus) * h * w // 16)
        criteria = OhemCELoss(weights=weight, ignore_label=ignore_label, thresh=0.7, n_min=min_kept)
    ...
    if args.dataset == 'camvid':
        ...
        optimizer = torch.optim.SGD(
            filter(lambda p: p.requires_grad, model.parameters()), args.lr, momentum=0.9, weight_decay=0.0005)
    elif args.dataset == 'cityscapes':
        optimizer = torch.optim.SGD(
            filter(lambda p: p.requires_grad, model.parameters()), args.lr, momentum=0.9, weight_decay=0.0005)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="MIDWRSeg", help="model name: [MIDWRSeg]")
    ...
    parser.add_argument('--input_size', type=str, default="360,480",
    ...
    parser.add_argument('--resume', type=str, default="./checkpoint/cityscapes/IDWRSeg_Lbs8gpu1_trainval/model_901.pth",
